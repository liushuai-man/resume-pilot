export const optimizationActionData = (input: {
  userId: string;
  resumeId: string;
  versionId: string;
  source: string;
...
export const optimizationActionDataView = (action: any) => ({
  ...
  canRevert:
    (action.action_type ?? 'suggestion') === 'suggestion' &&
    action.status === 'accepted' &&
    Boolean(action.version_id),
  createdAt: action.created_at,
  updatedAt: action.updated_at,
});

export function issueFromOptimizationAction(action: {
  field_id: string;
  final_text: string;
}) {
