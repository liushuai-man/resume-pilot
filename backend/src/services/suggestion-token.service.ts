...
const payloadSchema = z.object({
  kind: z.literal('resume_optimization_suggestion'),
  userId: z.string().uuid(),
  resumeId: z.string().uuid(),
  source: z.enum(['content_quality', 'job_match', 'ats']),
...
export function createSuggestionToken(
  payload: Omit<SuggestionTokenPayload, 'kind'>
) {
  return jwt.sign(
    { kind: 'resume_optimization_suggestion', ...payload },
