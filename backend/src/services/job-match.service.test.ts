...
const profile = {
  responsibilities: [{ name: '建设交易系统', evidence: '负责交易系统建设' }],
  required_skills: [{ name: 'Java', evidence: '熟悉 Java' }],
  preferred_skills: [],
};
      ...
      data: [
        {
          id: 'e1',
          position: '后端工程师',
          description: ['使用 Java 建设交易系统，将延迟降低 30%。'],
        },
      ],
    },
  ],
...
const output = JSON.stringify({
  dimensions: [
    { key: 'skillCoverage', score: 30, confidence: 0.9, reason: '技能有证据' },
    {
      key: 'responsibilityRelevance',
      score: 25,
      confidence: 0.9,
      reason: '职责相关',
    },
    ...
    { key: 'seniorityFit', score: 5, confidence: 0.5, reason: '年限信息不足' },
  ],
  requirements: [
    {
      requirementId: 'responsibility:0',
      status: 'matched',
      resumeFieldId: 'experience:e1:description',
      resumeEvidence: '建设交易系统',
      reason: '职责直接相关',
      confidence: 0.9,
    },
    {
      requirementId: 'required_skill:0',
      status: 'matched',
      resumeFieldId: 'experience:e1:description',
      resumeEvidence: 'Java',
      reason: '有使用场景',
...
test('拒绝编造的简历证据', () => {
  ...
  invalid.requirements[0].resumeEvidence = '不存在的证据';
  assert.throws(
    () => parseJobMatchOutput(JSON.stringify(invalid), requirements, fields),
    /证据无法定位/
  );
});
...
test('低置信度结论由后端强制归一化为待确认', () => {
  const invalid = JSON.parse(output);
  invalid.requirements[0].confidence = 0.4;
  const result = parseJobMatchOutput(
