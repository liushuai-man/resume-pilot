...
export function resolveAtsOptimizationField(content: unknown, issue: AtsIssue) {
  ...
  if (!['basic', 'education', 'skills', 'objective'].includes(section))
    throw new Error('该 ATS 问题需要结构化编辑，不支持 AI 自动改写');
  if (!issue.itemId && !['basic', 'objective'].includes(section))
    throw new Error('该 ATS 问题没有唯一字段落点');
  const fieldId = `${section}:${issue.itemId || 'root'}:${issue.field}`;
  const field = extractResumeQualityFields(content).find(
