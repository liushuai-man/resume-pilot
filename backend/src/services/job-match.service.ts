...
import { buildJobMatchPrompt, JOB_MATCH_EVALUATOR_VERSION, JOB_MATCH_PROMPT_VERSION, type MatchRequirement } from '../ai/prompts/job-match.prompt';
import { extractResumeQualityFields } from './content-quality.service';

const keys = ['skillCoverage', 'responsibilityRelevance', 'keywordEvidence', 'seniorityFit'] as const;
const maxScores = { skillCoverage: 35, responsibilityRelevance: 30, keywordEvidence: 20, seniorityFit: 15 } as const;
const schema = z.object({
  dimensions: z.array(z.object({ key: z.enum(keys), score: z.number().int().min(0), confidence: z.number().min(0).max(1), reason: z.string().min(1).max(500) })).length(4),
  requirements: z.array(z.object({ requirementId: z.string(), status: z.enum(['matched', 'insufficient_evidence', 'gap', 'needs_confirmation']), resumeFieldId: z.string().nullable(), resumeEvidence: z.string().nullable(), reason: z.string().min(1).max(500), confidence: z.number().min(0).max(1) })).max(150),
...
export function buildMatchRequirements(profile: any): MatchRequirement[] {
  const groups = [['responsibility', profile.responsibilities], ['required_skill', profile.required_skills], ['preferred_skill', profile.preferred_skills]] as const;
  return groups.flatMap(([category, values]) => list(values).map((item, index) => ({ id: `${category}:${index}`, category, name: String(item.name || ''), jdEvidence: String(item.evidence || '') })));
}

export function parseJobMatchOutput(output: string, requirements: MatchRequirement[], fields: ReturnType<typeof extractResumeQualityFields>) {
  const parsed = schema.parse(json(output));
  const dimensionMap = new Map(parsed.dimensions.map((item) => [item.key, item]));
  ...
  return { score: dimensions.reduce((sum, item) => sum + item.score, 0), dimensions, requirements: results, overallConfidence: parsed.overallConfidence, promptVersion: JOB_MATCH_PROMPT_VERSION, evaluatorVersion: JOB_MATCH_EVALUATOR_VERSION };
}

export async function evaluateJobMatch(userId: string, profile: any, resumeContent: unknown, options?: { maxRetries?: number; timeout?: number }) {
