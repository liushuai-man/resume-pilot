...
const content = {
  experience: [{ id: 'e1', description: '负责接口优化。' }],
  _documentSections: [
    { type: 'experience', data: [{ id: 'e1', description: '负责接口优化。' }] },
  ],
};
...
test('同时更新标准内容和编辑器骨架且不修改原对象', () => {
  ...
  assert.equal(
    updated._documentSections[0].data[0].description,
    '通过缓存优化接口。'
  );
...
test('字段已变化时拒绝覆盖', () => {
  assert.throws(
    () =>
      applyContentQualitySuggestion(
        { ...content, experience: [{ id: 'e1', description: '用户已修改' }] },
        issue,
        '新文本'
      ),
    /字段已变化/
