...
const contextOf = (value: unknown) =>
  value && typeof value === 'object'
    ? (
        value as {
          interviewContext?: {
            jobProfile?: { id?: string; jobTitle?: string } | null;
          };
        }
      ).interviewContext
    : undefined;
...
export async function getUserProfileOverview(userId: string) {
  const [user, rawInterviews, rawContent, rawMatches, rawActions] =
    await Promise.all([
      ...
      prisma.resumeContentAnalysis.findMany({
      ...
      prisma.resumeOptimizationAction.findMany({
  ...
  const scoredActions = actions.filter(
    (item) => item.score_before != null && item.score_after != null
  );
  const recentActivity = [
    ...interviews
      ...
      .map((item) => ({
        id: item.id,
        type: 'interview',
        title: `${item.position}面试报告`,
        detail: `${item.score} 分`,
        date: item.completed_at || item.created_at,
        href: `/interviews/results/${item.id}`,
      })),
    ...content
      .slice(0, 3)
      .map((item) => ({
        ...
        href: `/resumes/${item.resume_id}/edit`,
      })),
    ...matches
      .slice(0, 3)
      .map((item) => ({
  ...
  return {
    ...
    interview: {
      ...
      averageScore: interviews.length
        ...
        : null,
      latestScore: trend.at(-1)?.score ?? null,
      trend,
    },
    resumeQuality: {
      latestContentScore: content[0]?.score ?? null,
      contentConfidence: content[0]?.overall_confidence ?? null,
      latestMatchScore: matches[0]?.score ?? null,
      matchConfidence: matches[0]?.overall_confidence ?? null,
      acceptedOptimizations: actions.length,
      resolvedOptimizations: actions.filter((item) => item.resolved).length,
      averageScoreDelta: scoredActions.length
        ? Math.round(
            scoredActions.reduce(
              (sum, item) => sum + item.score_after! - item.score_before!,
              0
            ) / scoredActions.length
          )
        : null,
    },
    capabilityProfile:
    ...
    positionProfiles: [...positionGroups.entries()].map(([key, group]) => ({
      key,
      label: group.label,
      interviewCount: group.interviews.length,
      averageScore: Math.round(
        group.interviews.reduce((sum, item) => sum + item.score, 0) /
          group.interviews.length
      ),
      latestScore: group.interviews.at(-1)?.score ?? null,
      capabilityProfile:
        group.interviews.length < 2
          ? []
          : [
              ...new Map(
                group.interviews.flatMap((interview) =>
                  dimensionsOf(interview.report)
            ...
            ].map(([dimensionKey, dimensionGroup]) => {
              group.interviews.forEach((interview) =>
                dimensionsOf(interview.report)
                  .filter(
                    (item) => item.key === dimensionKey && item.questionCount
                  )
                  .forEach((item) => {
                    dimensionGroup.values.push(item.score);
                    dimensionGroup.evidenceCount += item.questionCount;
                  })
              );
              return {
                key: dimensionKey,
                ...
                score: Math.round(
                  dimensionGroup.values.reduce((sum, score) => sum + score, 0) /
                    dimensionGroup.values.length
                ),
                interviewSamples: dimensionGroup.values.length,
                evidenceCount: dimensionGroup.evidenceCount,
              };
            }),
    })),
    ...
    funnel: {
      ...
      confirmedProfiles: confirmedProfileCount,
      matchAnalyses: matches.length,
      interviewsStarted: interviews.length,
      reportsCompleted: interviews.length,
    },
