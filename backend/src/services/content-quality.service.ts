...
export function extractResumeQualityFields(
  input: unknown
): ResumeQualityField[] {
  ...
  const add = (
    ...
    const contentText = Array.isArray(value)
      ? value.map(text).filter(Boolean).join('\n')
      : text(value);
    if (!contentText) return;
    fields.push({
      fieldId: `${section}:${itemId || 'root'}:${field}`,
      section,
      itemId,
      field,
  ...
  skills.forEach((item, index) =>
    add(
      'skills',
      typeof item === 'string' ? String(index) : text(item.id) || String(index),
      'name',
...
export function parseContentQualityOutput(
  output: string,
  fields: ResumeQualityField[]
): Omit<ContentQualityResult, 'modelName'> {
  ...
  if (uniqueDimensions.size !== 5)
    throw new Error('内容质量评价维度不完整或重复');
  const dimensions = Object.entries(dimensionMaxScores).map(
    ([key, maxScore]) => {
      const dimension = uniqueDimensions.get(
        key as keyof typeof dimensionMaxScores
      );
      if (!dimension || dimension.score > maxScore)
        throw new Error(`内容质量维度 ${key} 分数无效`);
  ...
  const issues = parsed.issues.map((issue) => {
    ...
    if (!field)
      throw new Error(`内容质量问题引用了不存在的字段 ${issue.fieldId}`);
    if (
      !normalizeEvidence(field.content).includes(
        normalizeEvidence(issue.evidence)
      )
    ) {
...
export async function evaluateResumeContent(
  ...
  const output = await llm
    ...
    .invoke(buildContentQualityPrompt(fields));
  return {
    ...parseContentQualityOutput(output, fields),
