import { ChatOpenAI } from '@langchain/openai';
import { getDefaultModelConfig, getDecryptedApiKey } from '../../services/model-config.service';

export async function getUserModelClientConfig(userId?: string) {
  if (!userId) throw new Error('需要登录后才能使用 AI 功能');

  const config = await getDefaultModelConfig(userId);
  if (!config) {
    throw new Error('请先在模型管理中添加并设为默认模型');
  }

  return config;
}

export async function createUserLLM(
  userId?: string,
  options?: { temperature?: number; maxTokens?: number; maxRetries?: number; timeout?: number; modelKwargs?: Record<string, unknown> }
) {
  const { temperature = 0.7, maxTokens = 1000, maxRetries, timeout = 100000, modelKwargs } = options || {};
  const config = await getUserModelClientConfig(userId);
  const apiKey = await getDecryptedApiKey(config);

  return new ChatOpenAI({
    modelName: config.model_name,
    openAIApiKey: apiKey,
    temperature,
    maxTokens,
    timeout,
    maxRetries,
    modelKwargs,
    configuration: {
      baseURL: config.base_url || undefined,
    },
  });
}
