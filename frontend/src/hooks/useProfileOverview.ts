...
export function useProfileOverview() {
  ...
  useEffect(() => {
    if (isGuest) {
      Promise.all([guestWorkspace.listResumes(), guestWorkspace.listJobs(), guestWorkspace.listInterviewResults()]).then(([resumes, jobs, results]) => {
        ...
        setData({
          ...
          resumeQuality: { latestContentScore: null, contentConfidence: null, latestMatchScore: null, matchConfidence: null, acceptedOptimizations: 0, resolvedOptimizations: 0, averageScoreDelta: null },
          ...
          funnel: { jobDescriptions: jobs.length, confirmedProfiles: jobs.filter((item) => item.latestProfile?.status === 'confirmed').length, matchAnalyses: 0, interviewsStarted: results.length, reportsCompleted: completed.length },
        });
