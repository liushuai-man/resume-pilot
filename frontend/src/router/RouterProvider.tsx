...
const renderRoutes = (routes: RouteConfig[]): React.ReactNode => {
  return routes.map((route, index) => {
    ...
    if (children) return <Route key={index} path={path} element={layout || (element ? <Suspense fallback={<RouteFallback />}>{element}</Suspense> : undefined)}>{renderRoutes(children)}</Route>;
    // 如果没有子路由但有布局
    if (!children && layout) {
      return (
