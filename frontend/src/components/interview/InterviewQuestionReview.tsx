...
interface Props { questions: Question[]; answers: Answer[]; evaluations: QuestionEvaluation[]; onImproveResume?: (question: Question) => void; onPractice?: (question: Question, evaluation: QuestionEvaluation) => void; }
export default function InterviewQuestionReview({ questions, answers, evaluations, onImproveResume, onPractice }: Props) {
  ...
  return <Paper shadow="sm" p="lg" radius="md" withBorder mb="md"><Group mb="md"><HelpCircle size={18} className="text-[#176B52]"/><Text fw={600}>逐题回顾</Text><Text size="xs" c="dimmed">共 {answers.length} 题</Text></Group>
    <div className="space-y-5">{answers.map((answer, index) => { const question = questions.find((item) => item.id === answer.questionId); if (!question) return null; const evaluation = evaluationByQuestion.get(question.id); return <section key={question.id} className="border-b border-[#E1E7E4] pb-5 last:border-0 last:pb-0">
      ...
      {evaluation?.feedback ? <div className="rounded-lg border-l-3 border-emerald-500 bg-emerald-50 p-3"><Text size="xs" c="dimmed" mb={2}>批量评价结果</Text><MarkdownContent content={evaluation.feedback}/>{(evaluation.knowledgeGap?.length || evaluation.followUpSuggestion) && <Group mt="sm" gap="xs"><Button size="xs" variant="light" onClick={() => onPractice?.(question, evaluation)}>针对薄弱项再练一场</Button>{question.sectionKey && <Button size="xs" variant="subtle" onClick={() => onImproveResume?.(question)}>回到简历补充证据</Button>}</Group>}</div> : <Text size="xs" c="dimmed">该题暂无正式评价结果。</Text>}
    </section>; })}</div>
  </Paper>;
}
