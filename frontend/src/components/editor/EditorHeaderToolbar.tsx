...
export default function EditorHeaderToolbar({ title, onSave, onExport, lastModified, isSaving = false, onTitleChange }: EditorToolbarProps) {
  ...
  return <div className="flex w-full items-center justify-between gap-3">
    <div className="flex min-w-0 items-center gap-3">
      ...
      <Badge variant="light" color={isSaving ? 'yellow' : 'teal'} size="sm" radius="sm" className="shrink-0">
        {isSaving ? <span className="flex items-center gap-1"><Loader size={10} />保存中...</span> : lastModified ? `已保存 ${lastModified}` : '未保存'}
      </Badge>
    </div>
    <div className="flex shrink-0 items-center gap-2">
      <Button size="xs" radius="md" color="teal" className="h-8 px-3" onClick={onSave} disabled={isSaving} loading={isSaving} leftSection={<Save size={14} />}>保存</Button>
