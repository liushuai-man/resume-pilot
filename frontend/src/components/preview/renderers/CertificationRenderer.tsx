...
export function CertificationRenderer({ section, style, variant, isHighlighted, onClick }: RendererProps) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {items.map((item) => (
          <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <h4 style={{ fontSize: descSize, fontWeight: 500, margin: 0, color: '#374151' }}>
                {item.name}
              </h4>
              {item.issuer && (
