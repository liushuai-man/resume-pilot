...
export interface PageFragment { sectionId: string; itemIds?: string[] }
export interface SectionMeasure { totalHeight: number; itemHeights: Map<string, number>; itemGap: number; baseHeight: number }
...
const height = (measure: SectionMeasure, ids: string[]) => ids.length && measure.itemHeights.size
  ? measure.baseHeight + ids.reduce((sum, id) => sum + (measure.itemHeights.get(id) || 0), 0) + Math.max(0, ids.length - 1) * measure.itemGap
  : measure.totalHeight;
...
export function paginateSections(sections: ResumeSection[], measures: Map<string, SectionMeasure>, availableHeight: number): PageFragment[][] {
  ...
  for (const section of sections) {
    ...
    while (index < ids.length) {
      ...
      if (pages[pages.length - 1].length && measure.baseHeight + first > availableHeight - used) nextPage();
      const chunk: string[] = []; let chunkHeight = measure.baseHeight; const remaining = availableHeight - used;
      ...
      pages[pages.length - 1].push({ sectionId: section.id, itemIds: chunk }); used += chunkHeight; if (index < ids.length) nextPage();
    }
  }
  return pages;
}
