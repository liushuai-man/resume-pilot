...
| 后端固定回归 | 46/46 通过 | ATS、内容质量、岗位匹配、优化应用、版本恢复、面试批量评价、报告聚合、输入哈希与节点重试策略 |
...
| 三岗位真实模型校准 | 3/3 通过 | Kimi-K2.7-Code；初级前端、中级 Java 后端、高级平台工程师均通过结构与证据校验 |
| 内容质量真实模型校准 | 5/5 通过 | 优秀、语义填充、乱码重复、虚构实体中立性、跨字段矛盾均落入预设区间 |
| 岗位匹配真实模型校准 | 4/4 通过 | 强匹配、关键词无证据、能力缺口、职级不匹配均命中预期结论与分数区间 |
| 优化建议真实模型校准 | 2/2 通过 | 内容质量与 ATS 建议只使用合成用户事实，未引入额外事实 |
| 模拟面试真实模型正常链路 | 通过 | 2 次动态出题、3 题一次批量评价、覆盖 3/3、确定性报告成功生成 |
| 面试失败节点人工注入 | 待集中测试 | 需要可用模型，按第 6 节逐项验证保存、出题、批量评价、汇总和发布失败 |

“自动验收通过”只覆盖表中明确标记通过的范围。浏览器交互和故障注入仍须按清单人工完成后，才能将 V2.0 标记为最终产品验收通过。
