...
- 应用关键优化前创建内部恢复快照，并通过优化历史支持评分对比、结果追踪和撤销；不建设通用版本管理界面
...
| 优化效果验证 | 通过优化记录、ATS 分数和岗位匹配度对比，验证每次修改是否有效并支持撤销                                        |
...
| 优化历史模块   | AI修改前自动创建版本，展示修改摘要、ATS/匹配度变化，支持版本对比和恢复                                                         | 页面顶部和版本抽屉         |
...
| 业务层     | User Service / Resume Service / Interview Service | 负责用户管理、简历管理、模板管理、优化记录与恢复、岗位匹配、模拟面试、PDF导出等核心业务逻辑处理                                            |
...
V2.0 的阶段拆分、优先级假设、验证结果和下一步计划统一维护在 [V2.0 开发路线图](../roadmap/v2-roadmap.md) 中。路线图属于动态文档，可以根据用户反馈、实现成本和评测数据频繁调整；本产品文档只在产品目标或范围发生变化时更新。
...
| 优化历史 | 应用 AI 建议前自动创建内部快照；能够展示修改摘要、ATS/匹配度变化，并撤销已应用优化 |
