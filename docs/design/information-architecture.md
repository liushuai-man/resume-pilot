...
全部一级业务页面对游客开放。登录只负责云端归属、同步和长期历史，不作为页面访问条件；游客数据、匿名 AI 配额与登录迁移规则以 [游客体验与权限边界方案](./guest-experience.md) 为准。
...
除公开展示页、登录授权页和 PDF 打印页外，所有业务页面均保留固定顶部导航。简历编辑器和面试进行态可在其下增加局部上下文栏；存在未保存简历或进行中面试时，跨模块导航必须先确认并说明状态保留规则。品牌、公开展示页、主题、导航视觉和组件规范统一以 [V2.0 UI 设计系统](./ui-design-system.md) 为准。
