...
| 模拟问答   | AI作为面试官发问         |
| 实时评分   | 从回答中提取证据，按评价量表进行分项评分并记录置信度 |
| 动态追问   | 根据能力覆盖率、知识缺口和评价不确定性决定下一题 |
...
- 快速规则评价仅作为模型异常时的降级反馈，不写入高置信度能力画像
...
下一题优先级由岗位权重、尚未覆盖程度、能力差距、评价置信度和追问价值共同决定。最终报告分别展示岗位胜任度、能力覆盖率和评价置信度。
...
| POST   | /interview/message         | 提交回答并执行评价工作流     |
| POST   | /interview/end             | 结束面试并生成岗位评价报告   |
...
| 面试评价 | 单题评价包含分项得分、回答证据、知识缺口和置信度；最终报告展示岗位胜任度、覆盖率与置信度 |
